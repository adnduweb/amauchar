...
// Class definition
var KTmanagerAccount = function() {


    var initApps = function() {

        var bouttonManager = document.querySelector('#accountManager');
        //console.log(bouttonManager);

        if (bouttonManager != undefined) {
            // open block account manager
            bouttonManager.addEventListener('click', function(e) {
                $('.wrap-infos-account').toggleClass('is-visible');
                if ($('.wrap-infos-account').hasClass('is-visible')) {
                    $('#btnAccount').addClass('is-open');
                    $('.wrap-icon-account-min').addClass('d-none');
                    $('.wrap-infos-account').removeClass('d-none');
                    $('.wrap-btn-account-demande').removeClass('d-none');
                    $('.kt-aside-menu .kt-menu__nav > .kt-menu__item.last-item').css('padding-bottom', '150px');
                } else {
                    $('#btnAccount').removeClass('is-open');
                    $('.wrap-icon-account-min').removeClass('d-none');
                    $('.wrap-infos-account').addClass('d-none');
                    $('.wrap-btn-account-demande').addClass('d-none');
                    $('.kt-aside-menu .kt-menu__nav > .kt-menu__item.last-item').css('padding-bottom', '80px');
                }
            });
        }
    }
    ...
    var initDevis = function() {
        ...
        bouttonManagerDevis.addEventListener('click', function(e) {
            var typeMsg = 'quote';
            var productsChosen = $('input[name="devis[]"]:checkbox:checked').map(function() {
                return $(this).val();
            }).get();
            var dataArray = { 'productsChosen': productsChosen };
            sendMailFromBtn(typeMsg, dataArray);
        });
    }

    var sendMailFromBtn = function(type, data) {

        var dataPost;
        if (data === null) {
            dataPost = { 'productsChosen': null };
        } else {
            dataPost = data.productsChosen;
        }

        var modalSelector = '';
        if (type === 'quote') {
            modalSelector = '#kt_modal_devis';
        } else if (type === 'appointment') {
            modalSelector = '#kt_modal_rdv';
        } else if (type === 'contact') {
            modalSelector = '#kt_modal_contact';
        }
        ...
        axios.post(system.sendMailAccountManager, packets)
            .then(response => {
                console.log(response);
                if (response.data.messages.success) {
                    toastr.success(response.data.messages.success);

                    $(modalSelector + ' .modal-footer .btn-primary').remove();
                    $(modalSelector + ' .modal-footer .btn-primary').remove();
                    $(modalSelector + ' .modal-body').html('<div class="row"><div class="col-md-12"><b><i class="fa fa-check text-success"></i> ' + response.data.messages.success + '</b></div></div>');
                    $(modalSelector + ' .modal-footer button[type="reset"]').html('Ok');
        ...
        // $.ajax({
        //     url: 'ajax/global/send-mail-account-manager',
        //     type: 'POST',
        //     data: 'token=Sdo5Js0y2YpqIFr7H89rNDc-8FeHzTbAf4LNKenK9dA&type=' + type + dataPost,
        //     dataType: 'json',
        ...
        //                 $(modalSelector + ' .modal-footer .btn-primary').remove();
        //                 $(modalSelector + ' .modal-footer .btn-primary').remove();
        //                 $(modalSelector + ' .modal-body').html('<div class="row"><div class="col-md-12"><b><i class="fa fa-check text-success"></i> Votre demande a été envoyée avec succès.</b></div></div>');
        //                 $(modalSelector + ' .modal-footer button[type="reset"]').html('Ok');
        //             } else {
        //                 var msgError = '';
        //                 if (data.error === 'select_at_least_one') {
        //                     msgError = 'Veuillez sélectionner au moins une option.';
        //                 } else if (data.error === 'try_again') {
        //                     msgError = 'Une erreur est survenue, veuillez réessayer.';
        //                 } else if (data.error === 'empty_msg') {
        //                     msgError = 'Veuillez entrer un message à envoyer à votre Account Manager.';
        //                 }
        //                 $(modalSelector + ' .modal-body .errorMsgAjax .col-md-12').html(msgError);
        //                 $(modalSelector + ' .form-group-last.kt-hide').removeClass('kt-hide');
        //             }
        //         }
        //     }
        // });
    ...
    var loadRequestLiveChart = function() {
        var $requestsNumber = $('#requestsNumber');
        var chartData = [];
        var time = 0;

        if ('prod' === 'dev') {
            requestLiveAjax();
            setInterval(requestLiveAjax, 120000);

        } else {
            setInterval(requestLiveAjax, 3000);
        }

        function requestLiveAjax() {
            $.ajax({
                url: '/dashboard/ajax/liveRequests',
                type: 'POST',
                dataType: 'json',
                data: {
                    ginfo: 1
                ...
                success: function(data) {
                    if (data.notify) {
                        showBootstrapNotify(data.notify.type, data.notify.title, data.notify.message);
                    }

                    if (data.response === 'success') {
                        var liveRequestsNumber = data.liveRequestsNumber;
                        $requestsNumber.html(numberWithCommas(liveRequestsNumber));

                        if (chartData.length < 1) {
                            $('#loaderRequestLive').empty();
                            for (var i = 0; i < 11; i++) {
                                time += 1;
                                chartData.push({
                                    x: time,
                                    y: liveRequestsNumber
                                });
                            }

                            requestLive(chartData);
                        } else {
                            var chart = $('#requestLive').data('chart');
                            time += 1;

                            chartJsAddData(chart, {
                                x: time,
                                y: liveRequestsNumber
                            });

                            setTimeout(function() {
                                chartJsRemoveData(chart);
                            }, 10);
                        }
                    }
                }
    ...
    // Dark mode toggler
    var initAsideToggle = function() {
        var aside = document.querySelector('#kt_aside_toggle');

        if (aside) {
            aside.addEventListener('click', function(e) {
                e.preventDefault();

                axios.get(this.getAttribute('data-kt-url'), {
                        [crsftoken]: $('meta[name="X-CSRF-TOKEN"]').attr('content')
                    })
                    .then(response => {
                        console.log('Aside Toogle !!')
                    });
            });
        }
    }
...
$(document).ready(function() {
    KTmanagerAccount.init();
});

// Webpack support
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    module.exports = KTmanagerAccount;
}
"use strict";

// Class definition
var KTAppsAdnduWeb = function() {

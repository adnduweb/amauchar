        ...
        window.Ci4DataTables["kt_table_<?= $name; ?>-table"] = $(table).DataTable({
            ...
            'columnDefs': [
                ...
                }, 
                {
                    ...
                    targets: 2,
                    render: function(data, type, full, meta) {
                        return '<a href="mailto:' + full.email + '" class="text-dark fw-bolder text-hover-primary fs-6"><span class="fw-bolder">' + full.email + '</span></a>';
                    }, 
                    orderable: true,
                    searchable: true  
                },
                { 
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        html += '<a href="javascript:;" data-kt-customer-update="active" data-status="' + dataStatus + '" data-uuid="'+ full.uuid+'"';
                ...
                
                {
                    ...
                    render: function(data, type, full, meta) {
                        var htmlContent = '';
                            ...
                            htmlContent +=  '<!--begin::Menu item-->';
                            htmlContent +=  '<div class="menu-item px-3">';
                            htmlContent +=  '<a href="#" class="menu-link px-3" data-id="' + full.uuid + '" data-kt-<?= $name; ?>-table-filter="delete_row">Delete</a>';
                            htmlContent +=  '</div>';
                            htmlContent +=  '<!--end::Menu item-->';
                            htmlContent +=  '</div>';
    ...
    const toggleToolbars = () => {
      ...
      var initCustomerActive = () => {
        ...
        window.jQuery('[data-kt-customer-update="active"]').on('click', function (e) {
            ...
            axios.post("<?= route_to('customer.customer.update') ?>", formData)
            .then( response => {
                Ci4DataTables["kt_table_<?= $name; ?>-table"].ajax.reload();
            })
            .catch(error => {}); 

...
var KTPagesList = function () {
        ...
        window.Ci4DataTables["kt_table_<?= $name; ?>-table"] = $(table).DataTable({
            ...
            'columnDefs': [
                    ...
                    render: function(data, type, full, meta) {
                        return '<div class="d-flex align-items-center">\
                                    ...
                                    <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">\
                                        <a href="' + current_url + '/edit/' + full.id + '">\
                                            <div class="symbol-label fs-3 bg-light-danger text-danger">' + (full.name).substr(0,1).toUpperCase() +  '</div>\
                                        </a>\
                                    </div>\
                                    <!--end::Avatar-->\
                                    <!--begin::User details-->\
                                    <div class="d-flex flex-column">\
                                        <a href="' + current_url + '/edit/' + full.id + '" class="text-gray-800 text-hover-primary mb-1">' + full.name + '</a>\
                                        <span class="text-gray-800 text-hover-primary mb-1">' + full.name2s + '</span>\
                                    </div>\
                                    <!--begin::User details-->\
                                </div>';
                    },
                ...
                }, 
                {
                    ...
                    targets: 2,
                    render: function(data, type, full, meta) {
                        return '<span class="text-dark fw-bolder text-hover-primary fs-6"><span class="fw-bolder">' + full.slug + '</span></span>';
                    }, 
                    orderable: true,
                    searchable: true  
                },
                { 
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        html += '<a href="javascript:;" data-kt-customer-update="active" data-status="' + dataStatus + '" data-id="'+ full.id+'"';
                ...
                
                {
                    ...
                    render: function(data, type, full, meta) {
                        var htmlContent = '';
                            ...
                            htmlContent +=  '<!--begin::Menu item-->';
                            htmlContent +=  '<div class="menu-item px-3">';
                            htmlContent +=  '<a href="#" class="menu-link px-3" data-id="' + full.id + '" data-kt-<?= $name; ?>-table-filter="delete_row">Delete</a>';
                            htmlContent +=  '</div>';
                            htmlContent +=  '<!--end::Menu item-->';
                            htmlContent +=  '</div>';
    ...
    const toggleToolbars = () => {
      ...
      // Active item
      var initCustomerActive = () => {

        window.jQuery('[data-kt-customer-update="active"]').on('click', function (e) {
            ...
            axios.post("<?= route_to('page.activate') ?>", formData)
            .then( response => {
                Ci4DataTables["kt_table_<?= $name; ?>-table"].ajax.reload();
            })
            .catch(error => {}); 

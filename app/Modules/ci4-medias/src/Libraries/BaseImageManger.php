...
namespace Amauchar\Medias\Libraries;

use CodeIgniter\Config\BaseConfig;
use CodeIgniter\Config\Services;
use CodeIgniter\Files\Exceptions\FileNotFoundException;
use CodeIgniter\Files\File;
//use Amauchar\Medias\Core\ImageManger;
// use Tatter\Handlers\Handlers;
use Amauchar\Core\Libraries\Util;
use Amauchar\Medias\Config\ImageManger as ImageMangerConfig;
use Amauchar\Medias\Exceptions\ThumbnailsException;
use Amauchar\Medias\Interfaces\ImageManagerInterface;
...
class BaseImageManger
{
	...
	public function create(string $input, string $output): self
	{
		...
		if(setting('Medias.formatWatermark') == 1 && !empty(setting('Medias.formatWatermark'))){

			if ( \Config\Services::image()
				
				->withFile($input)
				->fit($this->width, $this->height, 'center')
				->text(setting('Medias.formatTextWatermark', ''), [            
					'color'      => '#fff',             
					'opacity'    => 0.5,            
					'withShadow' => true,            
					'hAlign'     => 'center',             

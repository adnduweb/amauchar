...
class MediaModel extends BaseModel
{
	...
	public function createFromFile(CIFile $file, array $data = []): Media
	{
		...
		$row['filename'] = Util::stringCleanUrl(pathinfo($row['filename'], PATHINFO_FILENAME))  . '.'. $row['ext' ];
		print_r($row);
		...
		{
			// Move the file
			$file = $file->move($storage . 'original/', $row['filename'] );
			chmod($storage . 'original/' . $row['filename'], 0664);
		}

		//Verify
		$row['clientname'] = Util::stringCleanUrl(pathinfo($row['clientname'], PATHINFO_FILENAME))  . '.'. $row['ext' ];
		...
		$inputOriginal   = $storage . 'original/' . $row['filename'];
		$outputThumbnail = str_replace('original', 'thumbnails', $file);
		$outputSmall     = str_replace('original', 'small', $file);
		$outputMedium    = str_replace('original', 'medium', $file);
		$outputlarge     = str_replace('original', 'large', $file);

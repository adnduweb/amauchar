...
?>
	...
	<div class="dropdown-menu" aria-labelledby="export-bulk">
		...
		<?php if ($export->ajax): ?>
		<input name="<?= $export->slug ?>" type="submit" class="dropdown-item" value="<?= $export->name ?>" onclick="$('#globalModal .modal-body').load('<?= site_url(ADMIN_AREA . '/medias/bulk/' . $export->slug) ?>'); $('#globalModal').modal(); return false;">
		<?php else: ?>

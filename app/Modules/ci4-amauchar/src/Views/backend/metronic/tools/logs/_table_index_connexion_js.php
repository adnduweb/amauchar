...
var KTLogsTrafficsList = function () {
    ...
    var initLogsTrafficsTable = function () {
        ...
        window.Ci4DataTables["kt_table_<?= $name; ?>-table"] = $(table).DataTable({
            ...
            'ajax': {
                'url':current_url + "/datatable-connexions",
                'data': {
                    // CSRF Hash
            ...
            'columnDefs': [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var status = {
							0: {'title': 'Error', 'state': 'btn-light-danger'},
							1: {'title': 'Success', 'state': 'btn-light-success'},
                        };
                        if (typeof status[full.success] === 'undefined') {
							return full.success;
                        }

                        var dataStatus =  full.success == 1 ? '' : 'disabled';
                        var html = '';
                        
                        html += '<span data-kt-user-update="active" data-status="' + dataStatus + '" data-uuid="'+ full.uuid+'"';
                        html += ' class="actionActive btn btn-bold btn-sm btn-font-sm ' + status[full.success].state + '">' + status[full.success].title + '</span>';
    ...
    var initToggleToolbar = () => {
        ...
        deleteSelected.addEventListener('click', function () {
            ...
            tRow = window.Ci4DataTables["kt_table_<?= $name; ?>-table"].rows($(this).closest("tr"));
            const ids = [];
            var dtRow = window.Ci4DataTables["kt_table_<?= $name; ?>-table"].rows('.selected').data().map(function(t, e) {
                ids.push(t.id);  
             });
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    axios.delete("<?= route_to(singular($name) . '.connexions.delete') ?>", { data: packets})
                    .then( response => {

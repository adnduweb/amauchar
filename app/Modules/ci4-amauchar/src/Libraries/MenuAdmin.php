...
class MenuAdmin {
    ...
    public $collection = ['modules', 'apps', 'system'];
    ...
    private function _generateItem($item, $level = 0) {
        ...
        //print_r(array_flip(service('uri')->getSegments())); exit;
        ...
        if(isset($item['path'])){
            ...
            if(isset($item['sub']['items']) && is_array($item['sub']['items'])){
                foreach($item['sub']['items'] as $it){
                    if(in_array($it['path'], service('uri')->getSegments())){
                        $show_hover = true;
                    }
                }
                if($show_hover == true){
                ...
                if(in_array($item['path'], service('uri')->getSegments())){
                    $classes[]= 'show hover';
                }
            }
        }
        ...
        echo '<' . $this->itemTag . ' ' . self::getHtmlAttributes($attributes) . self::getHtmlClass($classes) . '>';
    ...
    private function _generateItemLink($item) {
        ...
        if ( isset($item['name']) ) {
            // Assign the page URL
            $attributes['href'] = site_url(route_to($item['name'] . '.index'));
            ...
            if (isset($item['new-tab']) && $item['new-tab'] === true) {
             ...
             if (isset($item['params'])) {
                $attributes['href'] = $attributes['href'] . $item['params'];
             }
        ...
        if ( isset($item['menu-link-classes']) ) {
            $classes[] = $item['menu-link-classes'];
            //unset($classes[0]);
        }

        echo '<' . $tag . self::getHtmlClass($classes) .  self::getHtmlAttributes($attributes) . '>';
    ...
    private function _generateItemLinkTitle($item) {
        ...
        if ( isset($item['classes']) && isset($item['classes']['title']) ) {
            $classes[] = site_url(ADMIN_AREA . '/' .  $item['classes']['title']);
        }
        ...
        echo '<span ' . self::getHtmlClass($classes) . '>';

        if ( isset($this->callbacks['title']) && is_callable($this->callbacks['title']) ) {
            echo call_user_func($this->callbacks['title'], $item, $item['title']);
        } else {
            $item['title'] = str_replace('__v__', service('theme')->getVersion() , $item['title']);
            echo lang($item['title']);
    ...
    private function _generateItemLinkIcon($item) {
        ...
        if ( isset($item['icon']) ) {
            ...
            if (is_array($item['icon'])) {
                echo  service('theme')->getSvg($item['icon'][$this->iconType]);
            } else {
                echo service('theme')->getSvg($item['icon']);
            }
    ...
    private function _generateItemSub($sub, $level) {
        ...
        echo '<' . $this->parentTag . ' ' . self::getHtmlClass($classes) . '>';

        if (isset($sub['view'])) {
             echo view('\Themes\backend\/'. service('settings')->get('App.theme_fo', 'name').'/' . $sub['view'], []);
        } else {
    ...
    private function _generateItemHeading($item) {
        ...
        if ( isset($item['heading']) ) {
            if ( isset($this->callbacks['heading']) && is_callable($this->callbacks['heading']) ) {
                ...
                echo '<h3 ' . self::getHtmlClass($classes) . '>';
                echo lang('Core.' . $item['heading']);
                echo '</h3>';
            }
        }
    }
        ...
        if ( isset($item['content']) ) {
            if(isset( $item['ancre'])){
                //if(has_permission( $item['ancre'] . '.index')){
                    echo '<div ' . self::getHtmlClass($classes) . '>';
                    echo $item['content'];
                    echo '</div>';
                //}   
            }else{
    ...
    private function _buildBreadcrumb($items, &$breadcrumb, $options, $level = 0) {
        ...
        foreach ( $items as $item ) {
            ...
            if ( isset($item['name']) ) {
                ...
                if (in_array($item['name'], service('uri')->getSegments())) {
                    $breadcrumb[] = array(
                        'title'  => $title,
                        'path'   => isset($item['path']) ? site_url(route_to($item['name']. '.index')) : '',
                ...
                if(isset($item['section'])){
                    if (in_array($item['section'], service('uri')->getSegments())) {
                         $breadcrumb[] = array(
                             'title'  => $title,
                             'path'   => '#',
                             'route'  => $item['name']. '.index',
    ...
    public function getBreadcrumb($options = array()) {
        $breadcrumb = array(array(
            'title' => 'Core.Home',
            'path' => site_url(route_to('dashboard.index')),
            'active' => false
        ));
        ...
        foreach ($this->items as $item) {
            ...
            if(!isset($item['classes'])){
                if(isset($item['module'])){
                    foreach ($item['module'] as $itemModule) {
                        $this->_generateItem($itemModule);
                    }
                }
            }
